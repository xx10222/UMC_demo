...
public class UserDao {
    ...
    // 해당 userIdx를 갖는 유저의 status 조회
    public GetStatusRes getStatus(int userIdx) {
        String getStatusQuery = "select status from User where userIdx = ?";
        int getUserParams = userIdx;
        return this.jdbcTemplate.queryForObject(getStatusQuery,
                (rs, rowNum) -> new GetStatusRes(
                        rs.getString("status")),
                getUserParams); // 한 개의 회원정보를 얻기 위한 jdbcTemplate 함수(Query, 객체 매핑 정보, Params)의 결과 반환
    }

    // 해당 userIdx를 갖는 유저의 팔로잉 목록 조회
    //Follow 테이블에서 userIdx가 해당 userIdx와 같으며, status가 'A'인 목록만 조회한다
    public List<GetUserRes> getFollowings(int userIdx) {
        String getFollowingQuery = "select * from User where userIdx in (select followIdx from Follow where userIdx=?)"; //User 테이블에 존재하는 모든 회원들의 정보를 조회하는 쿼리
        int getUserParams = userIdx;
    ...
    // 해당 userIdx를 갖는 유저의 팔로워 목록 조회
    //Follow 테이블에서 followIdx가 해당 userIdx와 같으며, status가 'A'인 목록만 조회한다
    public List<GetUserRes> getFollowers(int userIdx) {
        String getFollowerQuery = "select * from User where userIdx in (select userIdx from Follow where followIdx=?);"; //User 테이블에 존재하는 모든 회원들의 정보를 조회하는 쿼리
        int getUserParams = userIdx;
    ...
    public int checkFollow(int userIdx, int followIdx) {
        //존재하면 1, 없으면 0 반환
        String checkFollowQuery = "select exists(select * from Follow where userIdx=? and followIdx=?)"; // Follow Table에 해당 팔로우 관계가 존재하는가?
        int checkFollowParams = followIdx; // 해당(확인할) followIdx 값
        int checkUserParams = userIdx; // 해당(확인할) userIdx 값
        return this.jdbcTemplate.queryForObject(checkFollowQuery,
                int.class,
                checkUserParams, checkFollowParams); // checkEmailQuery, checkEmailParams를 통해 가져온 값(intgud)을 반환한다. -> 쿼리문의 결과(존재하지 않음(False,0),존재함(True, 1))를 int형(0,1)으로 반환됩니다.
    }

    // 팔로우
    // userIdx와 followIdx가 User 테이블에 존재하는지 유효성 검사도 필요
    public int addFollow(PostFollowReq postFollowReq) {
        String createFollowQuery = "insert into Follow (userIdx, followIdx) VALUES (?,?)"; // 실행될 동적 쿼리문
        Object[] createFollowParams = new Object[]{postFollowReq.getUserIdx(), postFollowReq.getFollowIdx()}; // 동적 쿼리의 ?부분에 주입될 값
        this.jdbcTemplate.update(createFollowQuery, createFollowParams);

        String isFollowQuery = "select exists(select * from Follow where userIdx=? and followIdx=?);"; // 팔로우 성공 시 1 반환
        return this.jdbcTemplate.queryForObject(isFollowQuery, createFollowParams, int.class);
    }

    // 언팔로우
    // userIdx와 followIdx가 User 테이블에 존재하는지 유효성 검사도 필요
    public int unFollow(DeleteFollowReq deleteFollowReq) {
        String deleteFollowQuery = "delete from Follow where userIdx=? and followIdx=?"; // 실행될 동적 쿼리문
        Object[] deleteFollowParams = new Object[]{deleteFollowReq.getUserIdx(), deleteFollowReq.getFollowIdx()}; // 동적 쿼리의 ?부분에 주입될 값
        this.jdbcTemplate.update(deleteFollowQuery, deleteFollowParams);

        String isFollowQuery = "select exists(select * from Follow where userIdx=? and followIdx=?);"; // 언팔로우 성공 시 0 반환
        return this.jdbcTemplate.queryForObject(isFollowQuery, deleteFollowParams, int.class);
    }
}
